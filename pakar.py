...
def _parse_umur_hst(raw: str | None) -> int | None:
    if not raw:
        return None
    try:
        umur = int(raw.strip())
        ...
        return None
    return max(1, min(umur, 365))


def _top_group(hasil: list[dict]) -> list[dict]:
    if not hasil:
        return []
    top_key = max((item['is_terbukti'], item['persen']) for item in hasil)
    return [item for item in hasil if (item['is_terbukti'], item['persen']) == top_key]


def _render_hasil(
    gejala_dipilih: list[str],
    umur_hst: int | None,
    kandidat_list: list[str] | None = None,
    kandidat_index: int = 0,
):
    hasil_semua = backward_chaining(gejala_dipilih, umur_hst)
    ...
    top_group = _top_group(hasil_semua)
    kandidat_list = kandidat_list or [item['kode'] for item in top_group]

    kandidat_map = {item['kode']: item for item in top_group}
    kandidat_index = max(0, min(kandidat_index, len(kandidat_list) - 1))
    kandidat_kode = kandidat_list[kandidat_index]
    kandidat = kandidat_map.get(kandidat_kode, top_group[0])

    perlu_klarifikasi = len(top_group) > 1 and bool(kandidat.get('gejala_kurang'))
    ...
    return render_template('hasil.html',
                           ...
                           gejala=GEJALA,
                           umur_hst=umur_hst,
                           total_hipotesis=len(hasil_semua),
                           klarifikasi=perlu_klarifikasi,
                           kandidat=kandidat,
                           kandidat_list=','.join(kandidat_list),
                           kandidat_index=kandidat_index)
...
def diagnosa():
    ...
    umur_hst = _parse_umur_hst(request.form.get('umur_hst'))
    return _render_hasil(gejala_dipilih, umur_hst)


@app.route('/klarifikasi', methods=['POST'])
def klarifikasi():
    gejala_dipilih = request.form.getlist('gejala')
    umur_hst = _parse_umur_hst(request.form.get('umur_hst'))
    aksi = request.form.get('aksi', 'skip')

    kandidat_list_raw = request.form.get('kandidat_list', '')
    kandidat_list = [kode for kode in kandidat_list_raw.split(',') if kode]
    try:
        kandidat_index = int(request.form.get('kandidat_index', '0'))
    except ValueError:
        kandidat_index = 0

    if aksi == 'tambahkan':
        gejala_tambahan = request.form.getlist('gejala_tambahan')
        gejala_dipilih = list(dict.fromkeys(gejala_dipilih + gejala_tambahan))
        return _render_hasil(gejala_dipilih, umur_hst)
    ...
    if kandidat_index >= len(kandidat_list):
        kandidat_index = len(kandidat_list) - 1

    return _render_hasil(gejala_dipilih, umur_hst, kandidat_list, kandidat_index)
